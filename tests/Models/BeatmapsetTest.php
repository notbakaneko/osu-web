...
class BeatmapsetTest extends TestCase
{
    ...
    public function testChangingOwnerDoesNotQualify()
    {
        ...
        $beatmapset->beatmaps()->where('playmode', 1)->first()->setMappers([$guest->getKey()], $natUser);
        $beatmapset->beatmaps()->where('playmode', 0)->last()->setMappers([$beatmapset->user_id], $natUser);
        $beatmapset->refresh();
